...
namespace Talent.Services.Profile.Controllers
{
    ...
    public class ProfileController : Controller
    {
        ...
        {
            // Gets the current user (talent) data and returns 'FirstName'
        ...
        {
            // Gets the current user (talent) data and returns 'FirstName' & 'CreatedOn'
            var userId = uid;
            var user = await _userRepository.GetByIdAsync(userId);
        ...
        {
            // Checks if current user is 'Talent', 'Employer' or 'Recruiter'
        ...
        {
            // Returns the 'Src' of the image, based on the supplied & saved GUID of the 'image' name
        ...
        {
            // Retrieves the posted file from 'Request.Form' and passes to the 'UpdateTalentPhoto' function

            var userId = _userAppContext.CurrentUserId;
            bool isProfilePhotoUploaded = false;

            // Retrieve the posted image...
            IFormFile file = Request.Form.Files[0];

            // Upload profile photo if there is a photo file is supplied
            if (file.Length > 0) { 
                isProfilePhotoUploaded = await _profileService.UpdateTalentPhoto(userId, file);
            }

            if (isProfilePhotoUploaded)
            {
                return Json(new { Success = true, Message = "Profile photo has been uploaded successfully" });
            }
            else
            {
                return Json(new { Success = false, Message = "Error while photo upload" });
            }
        ...
        {
            // Retrieves the 'Employer Profile' based on current logged-in "user id" & "user role"...
            try
            {
        ...
        {
            // Updates the current employer record which eventually replaces the existing whole record with new record
        ...
        {
            // Updates the 'Client Profile' in the 'Recruiter data' 
            if (ModelState.IsValid)
            {
                // Retrieve all the 'Clients' list of the current logged-in user
                var recruiterClients =(await _recruiterRepository.GetByIdAsync(_userAppContext.CurrentUserId)).Clients;
               
                //check if employer is any existing client (for example, check for employer id: 5be40d789b9e1231cc0dc51b)
                if (recruiterClients.Select(x => x.EmployerId == employer.Id).FirstOrDefault())
                {
                    // Update the 'Client' record to 'Employer' 
        ...
        {
            // Get Talent Profile Record based on either supplied user id to this function or 
            // Retrieve the user id from currently logged-in user...
            try
            {
        ...
        {
            // Updates the Talent Profile record by replacing the whole existing record with new supplied record
            if (ModelState.IsValid)
            {
        ...
        {
            // Retrieve the 'Snapshot' rather than the whole record
            try
            {
        ...
        {
            // Retrieves the complete list of 'Talents'
            try
            {
        ...
        {
            // Retrieves the complete list of 'Employers'
            try
            {
        ...
        {
            // Retrieves the complete list of "employers" based on the supplied values in 'SearchCompanyModel'
            // Eventually it filters the data and returns the data based on the supplied values in 'SearchCompanyModel'...
            try
            {
        ...
        {
            // Retrieves the complete list of "talents" based on the supplied values in 'SearchTalentModel'
            // Eventually it filters the data and returns the data based on the supplied values in 'SearchTalentModel'...
            try
            {
        ...
        {
            // Retrieves the list of suggestions based on 'employer' or 'job' id for the 
        ...
        {
            // Adds the supplied suggested talent (to the employer record) 
            try
            {
        ...
        {
            // Retrieves the clients list of the current logged-in user
            try
            {
